...
public class PanelEstadisticas extends JPanel {
    ...
    private void inicializarBD() {
        try (Connection conn = DriverManager.getConnection(DB_URL)) {
            ...
            String sql = "CREATE TABLE IF NOT EXISTS Estadisticas (" +
                    ...
                    "plus80 INTEGER DEFAULT 0, " + // Cambiado de +80
                    "plus70 INTEGER DEFAULT 0, " + // Cambiado de +70
                    "intercepcion INTEGER DEFAULT 0, " +
                    "atajadas INTEGER DEFAULT 0, " +
                    "penalAtajado INTEGER DEFAULT 0, " +
                    "jugadorPartido INTEGER DEFAULT 0, " +
                    "entradas50 INTEGER DEFAULT 0, " +
                    "pasesClave INTEGER DEFAULT 0, " +
                    "plus1xG INTEGER DEFAULT 0, " + // Cambiado de +1xG
                    "recuperados INTEGER DEFAULT 0, " +
                    "FOREIGN KEY (jugador_id) REFERENCES Jugadores(id) ON DELETE CASCADE)";
            stmt.execute(sql);
        } catch (SQLException e) {
            mostrarError("Error inicializando la base de datos: " + e.getMessage());
        }
    }
    ...
    private JPanel createFormPanel() {
        ...
        btnAgregar.addActionListener(e -> agregarEstadisticas());
